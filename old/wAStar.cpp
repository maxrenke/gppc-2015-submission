...
//Return octile distance between two xyLoc //TODO diagonal?
	...
	while( !open_list.empty() ){
		...
		for( int i=0; i<succ.size(); i++){
			...
			if( succ[i].x == curr->pos().x || succ[i].y == curr->pos().y ){
				nb->set_values(curr->g()+1.0f,distance(succ[i],g),succ[i]);
			}
			else{
				nb->set_values(curr->g()+sqrt(2),distance(succ[i],g),succ[i]);
			}
